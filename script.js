...
const signupProcess = () => {
  ...
  if(parseInt(enteredOTP) === generatedOTP) {
    ...
    let exitingUsername = usersArray.find(user => user.username === username);
    if(exitingUsername){
      alert(`${username} is Already exit try another name `)
      return;
    }
    ...
    alert(`Signup Success! ✅ Welcome ${username}! Total users: ${usersArray.length}`);
...
const loginProcess = () => {
  ...
  let foundUser = usersArray.find(user => 
    user.email === loginUserGmail && user.username === loginUserName
  );
...
loginFunctinality();
signupFunctionality();
>>>>>>> cdb24c458c626c5a8ef04a20e6eef313ebc26520
